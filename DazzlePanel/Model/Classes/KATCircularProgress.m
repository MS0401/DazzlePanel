...
- (void)drawArcAnimation:(CGRect)rect {
    ...
    if (self.itemIndex == (self.sliceItems.count - 1)) {
        angle = (item.itemValue/total) * 2 * M_PI - 0.4;
    }else {
        angle = (item.itemValue/total) * 2 * M_PI;
    }
    ...
    self.startAngle = endAngle + 0.09;
   
    CAShapeLayer *layer = [CAShapeLayer layer];
    layer.path = path.CGPath;
    layer.strokeColor = item.itemColor.CGColor;
    layer.fillColor = nil;
    layer.lineWidth = item.itemWidth;//self.lineWidth;
