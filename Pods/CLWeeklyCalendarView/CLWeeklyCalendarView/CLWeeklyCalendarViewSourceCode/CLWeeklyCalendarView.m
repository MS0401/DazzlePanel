...
#define DATE_VIEW_MARGIN 15.f
#define DATE_VIEW_HEIGHT 28.f

#define DATE_LABEL_MARGIN_LEFT 9.f
#define DATE_LABEL_INFO_WIDTH 250.f
...
-(UIView *)dailyInfoSubViewContainer
{
    if(!_dailyInfoSubViewContainer){
        _dailyInfoSubViewContainer = [[UIView alloc] initWithFrame:CGRectMake(0, DATE_TITLE_MARGIN_TOP+DAY_TITLE_VIEW_HEIGHT + DATE_VIEW_HEIGHT + DATE_VIEW_MARGIN, self.bounds.size.width, DATE_LABEL_INFO_HEIGHT)];
...
-(void)initDailyViews
{
    ...
    self.end_DateTitle = dailyWidth*6;
...
-(UILabel *)dayTitleViewForDate: (NSDate *)date inFrame: (CGRect)frame
{
    ...
    if (frame.origin.x == 0 || frame.origin.x == self.end_DateTitle) {
        dayTitleLabel.textColor = [UIColor grayColor];
    }
    else {
    ...
    dayTitleLabel.text = [[[date getDayOfWeekShortString] substringToIndex:1] uppercaseString];
